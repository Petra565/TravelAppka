...
function Menu({ viewedPlaces, setIndexToOpen }) {
    ...
    return (
        <aside className="bg-[#ffb703] absolute z-600 right-2 top-54 px-6 py-6 rounded-xl  ">
            <TabGroup>
                <TabList className="grid grid-cols-2 gap-4">
                    <Tab className="col-span-1 border border-[white] text-[white] hover:bg-[#CC9200] data-selected:text-[#ffb703] data-selected:bg-white rounded-xl px-4 py-2 cursor-pointer">Navštívené miesta</Tab>
                ...
                <TabPanels>
                    <TabPanel className="mt-6 grid grid-cols-2 text-white">
                        <h1 className="col-span-1 text-center text-xl mb-4">Miesto</h1>
                        <h1 className="col-span-1 text-center text-xl mb-4">Počet dní</h1>
                        <section className="listOfViewedPlaces grid col-span-full grid-cols-2 max-h-[480px] overflow-y-auto">
                            {viewedPlaces.map((place, index) => (
                                <div
                                    key={index}
                                    className="grid col-span-full py-1 grid-cols-2 text-center bg-white text-sky-400 rounded-xl mb-4 cursor-pointer hover:bg-sky-400 hover:text-white"
                                    onClick={() => setIndexToOpen(index)}
                                >
                    ...
                    <TabPanel className="mt-6 grid grid-cols-4 gap-4">
                        <input
                            type='text'
                        ...
                        {plannedPlaces.length > 0 &&
                            <section className="list bg-white grid col-span-full grid-cols-4  py-2 rounded-sm grid-cols-2 max-h-[466px] overflow-y-auto">
                                ...
                                {plannedPlaces.map((place, index) => (
                                    ...
                                    >
                                        <div
                                            className="col-span-4 text-sky-400 border-b border-b-gray-400 font-[Corinthia] text-4xl"
                                        >
                                            {place}
                                        </div>
